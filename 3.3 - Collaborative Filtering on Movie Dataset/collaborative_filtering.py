...
class CollaborativeFiltering(object):
    ...
    def get_cosine_similarity(self, vector1, vector2):
        ...
        if isinstance(vector1, pd.DataFrame):
            return numerator.sum(axis=1) / (np.sqrt(vector1.sum(axis=1))*np.sqrt(vector2.sum()))
        elif isinstance(vector2, pd.DataFrame):
            return numerator.sum(axis=1) / (np.sqrt(vector1.sum())*np.sqrt(vector2.sum(axis=1)))            
        else:
            return numerator.sum() / (np.sqrt(vector1.sum())*np.sqrt(vector2.sum()))
    ...
    def get_k_similar(self, data, vector):
        ...
        row_mean = self.get_row_mean(data)
        normalized_data = self.normalize_data(data, row_mean)
        v_row_mean = self.get_row_mean(vector)
        vector = self.normalize_data(vector, v_row_mean)
        ...
        sim_n_v = self.get_cosine_similarity(normalized_data, vector)
        ...
        top_k = sim_n_v.nlargest(self.k).index

        # TODO: Return 2 values. See function comment
        return top_k, sim_n_v[top_k]
    ...
    def get_rating(self, data, index, column):
        ...
        top_k = self.get_k_similar(new_data, vector)


        # TODO: Compute for the rating using the similarity values and the raw
        # ratings for the k similar items.
        raw_data = new_data.loc[top_k[0]].iloc[:, column]
        rating = (raw_data * top_k[1]).sum() / top_k[1].sum()
        return rating
